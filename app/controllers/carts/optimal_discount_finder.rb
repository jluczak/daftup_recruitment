module Carts
  class OptimalDiscountFinder
    def call
      ...
      sets, extras = [], []
      best_choice["used_discounts"].each{|discount|
    ...
    def find_optimal_combination()
      ...
      combination_sums["sum"] = regular_price
      combination_sums["used_discounts"] = []
      combination_sums["regular_price_products"] = all_items.map{|item| item.product}
      ...
      if possible_discounts.any?
        ...
        discount_combinations.each{|combination|
          ...
          regular_price_products = (items_without_discounts + items_left).map{|item| item.product}

          if sum < combination_sums["sum"]
            combination_sums["sum"] = sum.round(2)
            combination_sums["used_discounts"] = used_discounts
            combination_sums["regular_price_products"] = regular_price_products
            combination_sums["regular_price"] = regular_price
          end
        }
      end
      combination_sums
